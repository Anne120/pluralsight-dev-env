<?php
  ...
  $new_amount = mysqli_real_escape_string($connect, $_POST['new_amount']);
  
  $updateQuery =mysqli_query($connect, "UPDATE land_price SET price = '$new_amount' ");
}
?>
<!doctype html>
<html lang="en">
...
<div class="container">
    ...
    <nav class="navbar navbar-inverse">
        ...
        <div class="navbar-header">
          ...
          <!-- <a class="navbar-brand" href="land_price.php"><span class="glyphicon glyphicon-dashboard"></span> Back to Land Price</a> -->
        </div>
  ...
  <div class="page-header">
   <!-- <h3 class="text-center">Update Land Prices</h3>   -->    
  </div>

  

<form method="post" action="land_price_two.php">
<div class="form-group">          
  <input type="text" name="new_amount" id="address" placeholder="Enter New Amount for Hectare Price" class="form-control" disabled>
</div>
<button type="submit" class="btn btn-success btn-block" style="margin-top: 28px; float: right;" name="update"disabled>Update Cost</button>
</form>
    
</div>
  <div class="col-sm-6">
    <div class="page-header">
    <h3 class="text-center">$ Price/ Ha</h3>      
  </div>
    <h3 class="text-center">
    <?php
        $Query = mysqli_query($connect, "SELECT * FROM land_price");
        while ($fetchDatabasePrice = mysqli_fetch_assoc($Query)) {
     ?>     
           <b> US $<?php echo $fetchDatabasePrice['price']; ?></b><br>
        <?php
      }
    ?>        
    </h3>
